...
- **JENIS RECORD DNS**: A Record    → hostname → IPv4 (contoh: example.com → 172.17.2.172)
...
- **CARA KERJA**: Buka browser example.com → browser query DNS untuk A Record
...
- **PRIVACY PROTECTION**: Tanpa proteksi → info asli pemilik domain tampil publik
  Dengan proteksi → nama perusahaan proxy yang tampil
                    contoh: Domains By Proxy, LLC (GoDaddy)

- **INFO YANG BISA DILIHAT**: Registrar, WHOIS Server, Creation Date,
...
- **HTTP METHODS**: GET    → ambil data dari server (HTML, gambar, dll)
...
- **AKSES MANUAL VIA TELNET**: telnet <IP> 80
...
- **CATATAN**: Type ascii TIDAK wajib untuk file .txt
...
- **COMMAND SMTP**: HELO/EHLO    → sapa server, mulai sesi SMTP
...
- **PENTING**: RCPT TO harus domain yang dikelola server tersebut
...
- **COMMAND POP3**: USER <username> → identifikasi user
...
- **KONEKSI PLAINTEXT**: telnet <IP> 110

KONEKSI SSL/TLS (jika server tolak plaintext) :
  openssl s_client -connect <IP>:995 -quiet

- **FLAG OPENSSL**: s_client          → mode SSL client (seperti telnet tapi terenkripsi)
...
- **BAHAYA PLAINTEXT**: Password & isi email bisa dibaca langsung via Wireshark/packet capture
...
- **INFO DARI SELECT inbox**: FLAGS          → status yang tersedia (\Seen \Answered \Draft dll)
