...
- **5 Tab MSConfig**: General  → Pilih mode startup Windows
...
- **3 Bagian**: Hardware Resources → Info teknis tingkat lanjut (bukan untuk user biasa)
...
- **Environment Variables**: Definisi : Variabel yang simpan info path/konfigurasi OS
- **Contoh**: %windir% = C:\Windows | %TEMP% = folder temporary
- **2 Jenis**: User variables   = berlaku untuk user tertentu saja
...
- **Search bar msinfo32**: Ada di bagian paling bawah
...
- **Tab Hardware**: Device Manager          → List semua hardware, ubah properties
  Device Install Settings → Pilih apakah Windows download driver otomatis

- **Tab Advanced**: Performance    → Visual effects, processor scheduling,
...
- **Info yang tersedia**: - Drive tempat page file disimpan
    - Initial size (MB)
    - Maximum size
...
- **Tipe dump yang tersedia**: Automatic memory dump
    Kernel memory dump
...
- **5 Tab**: Overview → Ringkasan semua 4 bagian sekaligus
...
- **Command Dasar**: hostname     → tampilkan nama komputer
...
- **Cara baca help**: command /?   = tampilkan manual (berlaku untuk SEBAGIAN BESAR command)
...
- **5 Hive Utama**: HKEY_CLASSES_ROOT    → File associations (.txt → Notepad, dll)
