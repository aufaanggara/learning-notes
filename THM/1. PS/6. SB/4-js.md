...
- **Breakdown**: Math.random()     → 0 sampai 0.999...
  * 20              → 0 sampai 19.999...
...
- **Tiga baris wajib**: import * as readline from "node:readline/promises";
  import { stdin as input, stdout as output } from "node:process";
- **const rl**: readline.createInterface({ input, output });

- **Penjelasan**: readline  = modul untuk tanya-jawab + tunggu jawaban
...
- **Pola**: /^\d+$/
- **Arti per bagian**: ^    = mulai dari awal teks
  \d   = satu karakter digit (0-9)
  +    = satu atau lebih
  $    = sampai akhir teks
  /  / = pembungkus regex di JavaScript

- **Penggunaan**: if (!/^\d+$/.test(text)) {
