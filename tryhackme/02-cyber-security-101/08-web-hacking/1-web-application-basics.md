...
- **2 Jenis**: HTTP Request  → Dikirim USER ke server (trigger aksi)
  HTTP Response → Dikirim SERVER ke user (balas request)

- **Struktur setiap message**: 1. Start Line  → Perkenalan pesan (jenis, tujuan, versi)
...
- **METHOD UTAMA**: GET     → Ambil data dari server (tanpa ubah apapun)
...
- **METHOD LAINNYA**: PATCH   → Update SEBAGIAN resource (bukan ganti total)
...
- **HTTP VERSION**: HTTP/0.9 (1991) → Hanya GET
...
- **5 KATEGORI**: 1xx Informational → Server terima sebagian, tunggu sisanya
...
- **KODE WAJIB HAPAL**: 100 Continue          → Server siap terima sisa request
  200 OK                → Sukses total
...
- **REQUIRED**: Date         → Waktu response dibuat server
...
- **COMMON**: Set-Cookie   → Kirim cookie ke browser untuk disimpan
...
- **Properti**: default-src → kebijakan default (biasanya 'self')
