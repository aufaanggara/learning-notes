...
- **Sintaks**: function namafungsi(parameter) {
      // isi kode
  }
  namafungsi(argumen);   ← cara memanggil

- **Contoh**: function PrintResult(rollNum) {
...
- **3 JENIS LOOP**: for        → Paling umum, iterasi dengan counter
  while      → Selama kondisi true, terus jalan
  do...while → Minimal jalan 1x, baru cek kondisi

- **Sintaks for**: for (let i = 0; i < 100; i++) {
...
- **Struktur lain**: switch → untuk banyak kondisi sekaligus
  switch (nilai) {
...
- **Contoh exploit**: for (let i = 0; i < 500; i++) {
...
- **PERBANDINGAN**: Kode asli    : 58 Bytes   → mudah dibaca
