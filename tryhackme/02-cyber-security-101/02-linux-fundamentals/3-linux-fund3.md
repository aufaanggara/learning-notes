...
- **Keunggulan**: - Customisable (shortcut bisa diubah sendiri)
...
- **Kirim file ke remote**: scp file.txt user@IP:/home/user/file.txt
- **Ambil file dari remote**: scp user@IP:/home/user/file.txt namalocal.txt
...
- **Format**: kill [PID]
- **Contoh**: kill 1337
- **Sinyal**: SIGTERM  = matikan proses, izinkan cleanup dulu
...
- **Option**: start    = jalankan service sekarang
...
- **Cara background**: [perintah] &    → langsung background, dapat PID
  Ctrl + Z        → pause & background proses yang sedang jalan

- **Cara foreground kembali**: fg              → bawa proses background kembali ke foreground
...
- **File penting**: /etc/apt/sources.list       = daftar repositori utama
...
- **Log penting**: apache2/access.log  = catat setiap request ke web server
