...
- **Contoh Cap**: /data/1  → data capture milik user sendiri
  /data/0  → data capture orang lain (HARUSNYA tidak bisa diakses!)
  Server tidak cek "ini data milik kamu?" → IDOR!

- **Kenapa susah dideteksi**: Request kelihatan normal & legitimate dari sisi server
...
- **Switches**: -u    = URL target (FUZZ = bagian yang diganti otomatis)
