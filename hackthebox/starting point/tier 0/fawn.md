...
- **Switch**: -sV   = service version detection (tau versi software)
...
- **Eksploit**: telnet [IP]          → konek ke target
...
- **Eksploit**: ftp [IP]             → konek ke server FTP
  Name: anonymous      → username anonymous
  Password: (enter)    → kosongkan password

- **Perintah di dalam FTP**: ls                   → lihat daftar file di server
...
- **Switch**: get   = download 1 file dari server ke lokal
